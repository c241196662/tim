...
public class Tim extends CordovaPlugin {
    ...
    public static final String ACTION_GETHISTORYMSG = "gethistorymsg"; // 获取历史消息
    ...
    private TIMConversation getconversation(CordovaArgs args) throws JSONException {
        ...
        try {
            params = args.getJSONObject(0);
            int conversationType = params.has("conversationType") ? params.getInt("conversationType") : 0;
            TIMConversationType ctype = TIMConversationType.Invalid;
            switch (conversationType) {
                case 1:
                    ctype = TIMConversationType.C2C;
                    break;
                case 2:
                    ctype = TIMConversationType.Group;
                    break;
            }
            //获取会话
    ...
    private void addMessageListener(CallbackContext callbackContext) {
        ...
        TIMManager.getInstance().addMessageListener(new TIMMessageListener() {//消息监听器
            ...
            public boolean onNewMessages(List<TIMMessage> msgs) {//收到新消息
                ...
                JSONArray msgjson = new JSONArray(msgs);
                try {
